...
void calcGlobal(int idx, std::vector<bool>& calculated, Avater& avater,
    ...
    if (node.parent >= 0) {
        ...
        // globalMtx[idx] = globalMtx[parent] * localMtx[idx]
        bx::mtxMul(
        ...
        // ルートはentityから
        // globalMtx[idx] = entityMtx * localMtx[idx]
        bx::mtxMul(
            avater.globalMtxs[idx].data(),
            entityMtx,
            localMtxs[idx].data()
...
void AvaterSystem::update(GameContext& ctx) {
    for (auto& avater : avaters) {
        ...
        // TRS順序: T * R * S
        bx::mtxMul(tmp2, r, s);      // tmp2 = R * S
        bx::mtxMul(entityMtx, t, tmp2); // entityMtx = T * (R * S) = T * R * S
